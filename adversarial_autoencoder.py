...
class ModularGenerator(Model):
    ...
    def add_prediction_op(self, input_logits=None, style_concat_input=None, **kwargs):
        with tf.variable_scope(self.config.model_name) as scope:
            ...
            # Actual Graph Construction for Prediction
            def add_prediction_op_helper(input_logits, style_concat_input):
            ...
            # Note: This weird try/catch structure is to enable variable reuse
            try:
                pred_output = add_prediction_op_helper(input_logits, style_concat_input)
            except ValueError:
