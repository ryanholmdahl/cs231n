...
class DC_WGAN():
    ...
    def loss(self, logits_real, logits_fake, real_imgs, generated_imgs):
        ...
        return discr_cost, g_cost

    def build(self):
        tf.reset_default_graph()

        self.x, self.y, self.global_step = self.add_place_holders()
        self.gen_images = tf.reshape(self.generator.add_prediction_op(preprocess_imgs(self.x)),
                                     shape=[-1, self.im_height, self.im_width, self.im_channels])

        with tf.variable_scope("") as scope:
            # scale images to be -1 to 1
            self.logits_real = self.discriminator.add_prediction_op(preprocess_imgs(self.y))
